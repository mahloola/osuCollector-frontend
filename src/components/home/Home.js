...
function Home({ user, setUser }) {
  ...
  const favouriteButtonClicked = (collectionId, favourited) => {
    ...
    setRecentCollections((recent) =>
      changeCollectionFavouritedStatus(recent, collectionId, favourited)
    )
    setPopularCollections((popular) =>
      changeCollectionFavouritedStatus(popular, collectionId, favourited)
    )
    if (favourited) {
    ...
    <Container className="pt-4 pb-4">
      <Row>
        <Alert variant="info" className="text-center">
          ...
          Join the <a href="https://discord.gg/WZMQjwF5Vr">osu!Collector discord</a>! Feel free to
          message FunOrange about any issues you have or suggestions for the site.
        </Alert>
        <Col className="px-5 my-2" md={12} lg={9}>
          <h2>Welcome to osu!Collector!</h2>
          <p>
            This is a place where you can view beatmap collections uploaded by other players. It is
            mainly developed by <a href="https://twitter.com/funorange42">FunOrange</a> and{' '}
          ...
          <Container className="p-2">
            <Row>
              {popularCollectionsError ? (
                ...
                <Container className="p-2">
                  <Row>
                    {(popularIsValidating ? new Array(6).fill(null) : popularCollections).map(
                      (collection, i) => (
                  ...
                  <LinkContainer to="/popular?range=week">
                    <Card
                      $lightbg
                      className="shadow-sm mt-1 mx-1 p-3 collection-card-clickable text-center"
                    >
      ...
      <Card className="my-4">
        <Card.Body>
          ...
          {recentCollectionsError ? (
            ...
            <Container className="p-2">
              <Row>
                {(recentIsValidating ? new Array(9).fill(null) : recentCollections).map(
                  (collection, i) => (
                    <Col lg={6} xl={4} className="p-0 my-3" key={i}>
                      <ReactPlaceholder
                        ready={collection}
